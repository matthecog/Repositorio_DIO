# Trabalhando com Azure Kubernetes Service (AKS e K8S)

## 📌 Status

✅ Concluído

## 🎯 Objetivo

Conhecer a arquitetura do AKS, Recursos de Workload e Redes

## 🚀 Conteúdos Abordados

Durante esta fase do bootcamp, foram estudados os seguintes conceitos:

* Containers e Docker
* Kubernetes (Pods, Deployments, Services, Ingress)
* Azure Kubernetes Service (AKS)
* Azure CLI e kubectl
* Namespaces e Contextos
* ConfigMaps e Secrets
* Auto Scaling (HPA e Cluster Autoscaler)
* Monitoramento com Azure Monitor e Log Analytics
* Boas práticas de segurança

## ☁️ Serviços Azure Explorados

- **Azure Kubernetes Service (AKS)**


## 🧪 Laboratórios Práticos

Durante o curso são realizados laboratórios para reforçar o aprendizado, incluindo:

* Deploy de aplicações
* Exposição de serviços

## 📌 Observações

- O ACR é essencial para o versionamento e distribuição de imagens